...
void pyty (RenderTexture & path, int x , int y, int xpre, int ypre, bool draw_line) // отрисовка траектории движения (типа путь)
{
    if(draw_line)
    {
...
//------------Подвес-----------//(устанавливается положение точки подвеса (если поставить в центр можно смотреть как по кругу колбасит)
    podv.x = (int)win_x/2;
...
//del позволяет начать заново отрисовку пути, O - выключить/включить
